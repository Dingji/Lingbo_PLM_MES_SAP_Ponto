...
public sealed class FileSyncProcessor
{
    ...
    public async Task<FileInfoResult> ProcessFileAsync(FileSyncItem item, CancellationToken cancellationToken = default)
    {
        ...
        result.AddStep("REQUEST_RECEIVED", $"FileCode={item.FileCode}, InventoryCode={item.InventoryCode}, FileName={item.FileName}");

        _logger.LogInformation(
            "[FileSync][RECEIVED] FileCode={FileCode} | InventoryCode={InventoryCode} | FileName={FileName} | Remark={Remark} | FileUrl={FileUrl} | RelativePath={RelativePath}",
            item.FileCode ?? "(null)", item.InventoryCode ?? "(null)", item.FileName ?? "(null)",
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(item.RelativePath))
            {
                ...
                if (!fullPath.StartsWith(Path.GetFullPath(_options.RootDirectory), StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    result.AddStep("SECURITY_BLOCKED", "Path traversal detected");
                ...
                {
                    result.Success = false;
                    result.ErrorMessage = "Local file not found";
        ...
        {
            result.Success = false;
                    result.ErrorMessage = "Download failed - check server logs for details";
                    result.AddStep("HTTP_ERROR", $"URL={item.FileUrl}");
            _logger.LogError(ex, "[FileSync][HTTP ERROR] FileCode={FileCode} | URL: {Url} | Error: {Error}", item.FileCode, item.FileUrl, ex.Message);
        }
        catch (TimeoutException ex)
        {
            result.Success = false;
            result.ErrorMessage = "Download timed out";
        ...
        {
            result.Success = false;
            result.ErrorMessage = "File I/O error occurred";
        ...
        {
            result.Success = false;
            result.ErrorMessage = "File is corrupted or invalid";
        ...
        {
            result.Success = false;
                    result.ErrorMessage = $"Unexpected error: {ex.GetType().Name}";
                    result.AddStep("UNEXPECTED_ERROR", $"{ex.GetType().Name}");
    ...
    /// Format: inventoryCode/fileName
    /// </summary>
    private static string BuildRelativePath(FileSyncItem item)
    {
        var folder = string.IsNullOrWhiteSpace(item.InventoryCode) ? "_default" : SanitizePath(item.InventoryCode);
    ...
    /// <summary>
    /// Recursively extracts archive entries (including archives nested inside archives) down to the
    /// deepest level using SharpCompress (supports ZIP, RAR, 7Z, TAR, GZ, BZ2, XZ, etc.).
    /// </summary>
    private async Task<List<ZipEntryInfo>> ExtractArchiveEntriesAsync(string archivePath, string archiveType, CancellationToken cancellationToken)
    {
        var leaves = await Task.Run(
            () => ArchiveExtractor.ExtractLeaves(archivePath, cancellationToken), cancellationToken);

        var entries = leaves.Select(leaf => new ZipEntryInfo
        {
            EntryName = leaf.EntryPath,
            UncompressedSizeBytes = leaf.Size,
            CompressedSizeBytes = leaf.Size,
            LastModifiedTimeUtc = leaf.LastModifiedUtc,
            Md5 = leaf.Md5,
            IsDirectory = false
        }).ToList();
