...
public class BomDebouncerService : BackgroundService
{
        ...
        ILogger<BomDebouncerService> logger)
    {
        ...
        _logger = logger;
        _delaySeconds = configuration.GetValue("SyncSettings:DelaySeconds", AppConstants.DefaultDelaySeconds);
    }
    ...
    public void EnqueueTrigger(TriggerEvent evt)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                _logger.LogInformation(
                    "[SHUTDOWN] BOM {BomId} | Sync cancelled due to application shutdown",
                    evt.Id);
            }
