...
public class FileSyncDebouncerService : BackgroundService
{
        ...
        ILogger<FileSyncDebouncerService> logger)
    {
        ...
        _logger = logger;
        _delaySeconds = configuration.GetValue("FileSyncSettings:DelaySeconds", AppConstants.DefaultFileSyncDelaySeconds);
    }
    ...
    public void EnqueueTrigger(TriggerEvent evt)
    {
        ...
        var cts = CancellationTokenSource.CreateLinkedTokenSource(_appStoppingToken);
        _pending[evt.Id] = cts;
        ...
        _ = Task.Run(async () =>
        {
            ...
            catch (OperationCanceledException) when (_appStoppingToken.IsCancellationRequested)
            {
                _logger.LogInformation(
                    "[SHUTDOWN] FILES {FileId} | Sync cancelled due to application shutdown",
                    evt.Id);
            }
